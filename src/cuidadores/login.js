...
const Login = () => {
  ...
  const { login } = useContext(AuthContext); //EXTRAER función login del contexto
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        login(data.cuidador); //llama login del contexto
        navigate("/dashboard");
      } else {
    ...
    <div className="min-h-screen flex flex-col bg-gray-100">
        ...
        <div className="relative">
          ...
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-40 bg-white border rounded-md shadow-lg z-50">
              <button onClick={() => navigate("/acerca")} className="block w-full text-left px-4 py-2 hover:bg-gray-100">Acerca de</button>
              <button onClick={() => navigate("/contacto")} className="block w-full text-left px-4 py-2 hover:bg-gray-100">Contacto</button>
            </div>
          )}
        </div>
      </header>
      ...
      <main className="flex flex-1 justify-center items-center px-4">
        <div className="bg-white w-full max-w-md p-6 rounded-lg shadow-md">
          <h1 className="text-2xl font-bold text-center text-gray-800 mb-2">AlzhTrack</h1>
          ...
          <div className="text-center text-sm text-gray-600 mt-4">
            <p className="mb-1 hover:underline cursor-pointer">¿Olvidaste tu contraseña?</p>
            <p>
